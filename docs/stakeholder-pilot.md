...
- All wired states, including NC and GA, run in one comparison batch with one dashboard refresh timer. Batch timestamps, failed states and per-state unchecked/excluded assignment counts are visible. Whole-read failures retain earlier findings with a warning. Partial batches omit failed states and name them explicitly.
