...
Status: **investigated 2026-08-26, not safe to build yet.** A+'s `XATXBD` `NY%` codes map essentially 1:1 to real jurisdictions (unlike GA/SC/CO, no address-boundary matching is needed to interpret an already-assigned tax body). But a direct comparison against the live, current NY DTF Publication 718 turned up **two confirmed, material, currently-live rate mismatches** (Suffolk County and Yonkers City) plus several code-numbering inconsistencies and orphaned codes on both sides. This is closer to CO's outcome (a live, unresolved mismatch found) than to NC's (clean match). Recommend a human decision before writing an adapter.
...
- **Do not build a naive code-based comparison yet.** A straight join on "A+ code number = Pub 718 reporting code number" would silently mis-join at least 4 rows (White Plains, and the 3-row Oneida cluster), and a straight rate comparison would flag Suffolk/Yonkers as the only two real discrepancies only if the join itself is right first.
- **Get a decision from Ana/Liv (or whoever owns the A+ tax-body master) on Suffolk and Yonkers specifically** before shipping any comparison — these look like genuine stale rates in A+ that a live customer could be paying the wrong tax rate on, not TaxAP data-quality noise. This is a stronger finding than most other states' investigations and probably worth surfacing regardless of when/whether the NY adapter gets built.
- If proceeding: match by name/place first (the way SC's adapter reads structure from `TBTXNAM` rather than trusting `TBTXBOD` numbering), not by assuming the numeric suffix equals Pub 718's reporting code — the Oneida cluster and White Plains prove that assumption breaks down in at least a few cases.
