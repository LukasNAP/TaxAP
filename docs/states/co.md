...
Status: **investigated 2026-08-26 (Step 1/2 only) — no adapter built, and not recommended yet without a human decision.** Confirmed live against `XATXBD` and cross-checked against the current official combined rate file. Colorado turns out to be the most structurally difficult state investigated so far: the address-matching need is more severe than GA/SC (a single city name can carry multiple valid state rates depending on which county/RTD/CD-district sliver an address sits in), and a live, currently-material rate mismatch was found on Denver — Atlantic's biggest CO market.
...
4. **Add an xlsx-parsing dependency (or a hardened parser)** before building a real `co-rates.mjs` adapter — the parser used for this investigation was a throwaway regex-based OOXML reader with no validation, not production-ready.
...
**Given all of the above, `recommendBuild = false` for CO right now** — not because the official source is unreachable (it's confirmed live and readable) or because Step 1/2 are unanswered (both have real, specific answers here), but because what they reveal is a genuinely harder matching problem than any state built so far, plus one live, material, unresolved rate discrepancy on Atlantic's biggest CO market. Building against this without a design decision on the multi-variant-per-city problem risks shipping a confidently wrong comparison — exactly the failure mode this project's rules exist to prevent.
