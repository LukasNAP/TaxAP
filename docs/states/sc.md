...
Status: **official rate source connected (2026-08-26); a real boundary/address-matching source has now been identified (2026-08-26) but not built.** `server/sc-rates.mjs` fetches and validates ST-575 live; `readOfficialScRates()` is wired into `/api/official/states/SC`. SC DOR itself publishes no address/ZIP-level data, but SC RFA's public GIS REST services (geocoder + municipal/county boundary polygons at `gis.state.sc.us`) do — see Step 3. Address-matching logic against `XATXBD`'s SC0xx codes remains unbuilt and unvalidated. Investigated live on 2026-08-25–26.
...
**Bottom line: a usable, government-published, address/ZIP-level boundary source for SC does exist** (SC RFA's public GIS REST services), which is more than SC DOR itself publishes. Census TIGER/Line (the fallback the research question asked about) was not pursued further once this was found — RFA's own layers are the *actual* SC state government's own municipal boundaries (used by the state's own public rate-lookup tool), which is strictly better-fitted to SC's specific wrinkles (James Island, straddling cities) than generic Census place polygons would be on their own, and TIGER would still need combining with the same ST-575 name list either way. This has not been built, and per this project's rules should not be treated as reliable until: the raw `TaxRate`/FIPS values are re-verified directly (not through a summarizing fetch), the `Municipalities`/`County_Boundary` name spellings are confirmed to match ST-575's spellings for all 294+46 real rows, the Charleston-style straddling cases are confirmed to resolve correctly via the two-layer (city + county) approach above, and RFA's terms of use/rate limits for programmatic (non-interactive) use of `gis.state.sc.us` are checked — none of that has been done yet.
