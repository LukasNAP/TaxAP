...
## 2026-08-27: new findings from building FL/PA/OH's actual A+ matching

Building the real per-tax-body reconciler for Florida and Ohio (not just re-reading the earlier investigation's manual spot-checks) surfaced a few confirmed live stale rates that weren't in the original findings lists above. Same standing default applies: no known Atlantic-specific exception, flag for A+ correction, pending Ana/Liv sign-off.

- **Florida — Marion County (`FL041`, 26 active ship-tos):** official rate 7.5% vs. A+ 7% (0.5pt understated). Not part of the original 4-county Florida finding (Collier/Flagler/Hamilton/Palm Beach) — found only once the full reconciler ran against every FL code, not just the originally spot-checked ones.
- **Ohio — Portage County (`OH067`, 12 ship-tos):** official rate 7% vs. A+ 7.25% (0.25pt *over*charged) even after correctly folding in Portage's transit-authority surcharge component. Direction is the opposite of most other findings in this memo (A+ too high, not too low).
- **Ohio — Tuscarawas County (`OH079`, 3 ship-tos):** official rate 6.75% vs. A+ 7.25% (0.5pt overcharged), with no matching transit-surcharge special-jurisdiction row found to explain the gap.
- **Ohio — Brown County (`OH008`, 2 ship-tos):** official rate 7% vs. A+ 7.25% (0.25pt overcharged), same shape as Tuscarawas.

Also confirmed while building this: Ohio's ~16-county transit-authority surcharge crosswalk is real and solvable — the official SST file's "special" jurisdiction row for a transit-surcharge county uses jurisdictionCode `<A+'s own OH### county number>000` (e.g. Franklin is `OH025` and its surcharge is code `25000`). Folding this in correctly cleared 10 counties (Cuyahoga, Franklin, Hamilton, Montgomery, Summit, Lucas, Stark, Portage's surcharge portion, Lake, Allen, Mahoning) that would otherwise have been false-flagged as stale by a naive county-row-only comparison — see `server/oh-aplus.mjs`.

Separately: Florida's `FL###` code is **not** a reliable alphabetical index into Florida's 67 counties, despite an earlier investigation claiming this after only 4 spot-checks. A full live run found `FL055` is really St. Johns (not the 55th county alphabetically) and `FL058` is really Sarasota (not the 58th) — matching by the real county name already in each code's own `TBTXNAM` (e.g. "Florida St. Johns") is the only safe approach, now what `server/fl-aplus.mjs` does. No business decision needed here — this is corrected engineering, not a new open question — but worth remembering before trusting any future "index-based" claim about another state without running it first.

## 2026-08-27 (same day, later): VA/NY/AZ/AL built and wired — prior findings now live, plus new ones

Virginia, New York, Arizona, and Alabama each moved from "decision resolved" to "actually built and visible in the shared dashboard inbox" via `server/direct-mapping-aplus.mjs`. All of the previously-resolved decisions above for these four states are now live, confirmed findings a user can see in TaxAP today, not just text in this memo:

- **Virginia:** Richmond (`VA076`→`VA216` miscoding, 57 ship-tos) is live. Building the full reconciler also surfaced a new one not in the original investigation: **Pittsylvania County** — official rate differs from A+'s configured rate, same "no known exception, flag for correction" treatment. Of VA's 133 real localities, the 4 name-duplicate pairs (Fairfax, Franklin, Richmond, Roanoke) all resolve correctly using the source workbook's own explicit "County"/"City" suffix.
- **New York:** Suffolk County and Yonkers City (both previously resolved above) are live. Sherrill/Fulton (Decision 3, zero active ship-tos) remain correctly excluded as dormant, not compared. No new stale rates surfaced beyond what was already found.
- **Arizona:** Douglas, Casa Grande, Taylor, and `AZ203`/City-of-Maricopa (all previously resolved above) are live. Building the full reconciler surfaced two more not in the original investigation: **Gilbert** and **Surprise** — same "no known exception, flag for correction" treatment. `AZ3518` (Green Valley, Decision 3) remains unmatched/unaddressed, consistent with "never guess." The 3 real name-duplicate city/county pairs confirmed live (City of Maricopa/Pinal, Town of Pima/Graham, City of Yuma/Yuma) are handled via a verified static crosswalk, not guessed.
- **Alabama:** Decision 2's "general sales rate only" scope is now the live filter. 26 confirmed stale-rate findings are visible in the dashboard (not individually itemized here — see the live inbox). **A new, honest limitation surfaced during the build, not previously known:** Alabama's numeric tax-body suffix is usually ADOR's own real locality code, but for multi-county cities (Birmingham, Hoover, Decatur, Dothan, Huntsville, Madison City, and others) A+ sometimes reuses a *different* county's own locality-code number for the city row — e.g. `AL7058` labeled "Birmingham(Shelby" actually carries Shelby County's own code, not Birmingham's real one. A name-cross-check safety net catches this and reports "unmatched" rather than guessing, which is why **29 of 106 AL codes are correctly reported unmatched** rather than matched — this is a known, documented gap (not everything could be resolved without either address-level data or splitting these cities' A+ codes by county side), not a bug to silently paper over.

A generic bug in the shared reconciler was also found and fixed while building AZ: `Number(null) === 0` was silently treating an official rate that failed to resolve (e.g. a county missing from a static crosswalk) as a confident "0% mismatch" instead of "unavailable." Fixed to require an explicit non-null/non-undefined official rate before calling anything matched — verified this caused no silent regression on FL/PA/OH/VA/NY's already-shipped results.

MO and CO remain investigated-but-not-built (see their resolved decisions above) — no Layer 1 official-source adapter exists for either yet, and each has enough of its own real complexity (MO's sparse non-standard code numbering; CO's already-known ambiguous multi-rate cities) to warrant its own dedicated build pass rather than a rushed one alongside VA/NY/AZ/AL.
