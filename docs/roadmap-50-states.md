...
|---|---|---|
| **1. Official source** | A validated, machine-readable feed of that state's current rates | 47 connected; 4 correctly marked no-general-sales-tax; 0 remain |
...
**Historical 2026-08-26 investigation:** AL, AZ, CO, LA, MO, NY, HI, and NM all exposed real state-specific complications. All now have official evidence adapters that preserve those complications without pretending the A+ comparison is settled. Alaska's current ARSSTC member rates are also connected with explicit partial-coverage labeling. See `docs/state-rollout.md` and each state's note before adding comparison logic.
...
| VA | `tax.virginia.gov` sales-tax-rate-and-locality-code-lookup | XLSX, connected in `server/va-rates.mjs` | Rechecked 2026-09-02: the official server's workbook is now `Last-Modified` 2026-01-09 and contains all 95 counties plus 38 independent cities across the current 5.3%, 6.0%, 6.3%, and 7.0% general-rate tiers. Official inventory is connected; address/locality and A+ exception handling remain separate. |
...
| State | Source | Format | Notes |
|---|---|---|---|
| AL | ADOR `taxrates_current.csv` (revenue.alabama.gov) | Monthly current-only CSV, connected in `server/al-rates.mjs` | September 2026 validation yields 823 `ST/GENER` locality/county rows and 258 explicit police-jurisdiction variants; matching remains withheld for the interactive-only address lookup, coarse A+ catalog, and unresolved `AL000`. |
...
| CO | Combined DR 1002 + DR 0800 spreadsheet (tax.colorado.gov) | Dynamically resolved half-year XLSX, connected in `server/co-rates.mjs` | Current 447-code layered inventory includes 92 self-collected-home-rule rows. Multi-county/district variants prohibit city-name matching; Denver and address-level/A+ decisions remain unresolved. |
| LA | Remote Sellers filing lookup + Louisiana DOR state rate | Current server-rendered parish tables, connected in `server/la-rates.mjs` | September 2026 crawl validates all 64 parish selectors and 438 composite domicile rates. Reused domicile codes remain parish-qualified; address and sparse A+ matching remain unresolved. |
| MO | Quarterly rate tables (dor.mo.gov) | Dynamically resolved XLSX, connected in `server/mo-rates.mjs` | Current July-September 2026 workbook validates 2,550 unique filing-code combinations and the 4.225% state component. Address-to-code and sparse A+ matching remain separate unresolved problems. |
...
| NM | tax.newmexico.gov GIS data download + UNM RGIS | Dynamically resolved ZIP containing CSV + XML metadata, connected in `server/nm-rates.mjs` | Current 2026-07-01 through 2027-06-30 inventory validates 248 polygons / 284 unique location codes, including paired tribal classes. GRT remains explicitly seller-side; address matching, A+ reconciliation, and the source's Lovington Industrial Park omission remain visible limitations. |
...
### Special case: Alaska — current member source connected, local-only, partial coverage

AK has **no state-level sales tax at all**. `server/ak-rates.mjs` now resolves and validates ARSSTC's current XLSX, exposing 56 participating remote-seller destination rows (10 borough-area and 46 city rows) as of 2026-09-01, plus the explicit 0% state row. ARSSTC covers **only its member jurisdictions**, so the adapter clearly marks nonmember municipality and address-boundary coverage unresolved. Alaska DCRA's annual Alaska Taxable/ArcGIS inventory is available as broader completeness evidence, but is not silently merged with the current member sheet. See `docs/states/ak.md`.
...
### Resolved since the August 26 source pass: D.C. is connected

The old `dc-tax-rates` page remains useful history, but it is no longer the adapter source. On 2026-09-03 the current OTR tax-law-change notice was live and explicitly confirmed that the general rate remains 6% through 2026-09-30 and becomes 7% on 2026-10-01. The intervening 6.5% step was delayed and never became the current rate. `server/dc-rates.mjs` now validates both percentages and dates, exposes the future change, and automatically rolls forward. The aggregate A+ audit found `DC000` at 0% for 20 active ship-tos plus one D.C. ship-to on `HN000`; see `docs/states/dc.md`.
...
7. **Resolved:** AL, AZ, CO, LA, and MO now have state-specific official-source adapters. Their address/A+ matching decisions remain separate and are documented per state.
8. **Resolved at the source layer:** NY's current Publication 718 adapter and MD's flat 6% official-document adapter are connected. New York's documented A+ discrepancies still require human decisions.
9. **Resolved product framing:** DE/MT/NH/OR are excluded because they have no general sales tax; HI GET and NM GRT are treated as functionally equivalent for monitoring while retaining their correct seller-side labels; AK is connected with explicit ARSSTC member-only coverage. The remaining work in those states is documented technical/data scope, not an unanswered framing decision.
10. **D.C. completed 2026-09-03:** the live OTR notice is connected with the current 6% rate and scheduled 7% transition; the remaining work is a business review of the zero-rate A+ assignments, not source discovery.
11. **Resolved:** every state/DC source is now connected or explicitly classified no-general-sales-tax.
12. **Next milestone:** matching work (layer 2) can proceed state by state, with address and business decisions preserved rather than guessed.
