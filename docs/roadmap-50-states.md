...
| **2. A+ jurisdiction matching** | Pulling that state's real `XATXBD` tax-body data and determining (a) whether it needs address matching or maps 1:1 to jurisdictions, and (b) whether A+'s rate is actually comparable to the official one | **Milestone (2026-08-26, later session): investigated (data pulled, findings documented) for all 47 states/DC that need it** — NC, GA, SC, NJ, AL, AZ, MO, CO, LA, NY, HI, IA, KS, ND, SD, VT, NM, MN, NE, NV, OK, UT, WA (first pass) plus AR, WY, CA, TX, FL, PA, OH, TN, MD, IN, KY, MI, RI, IL, VA, WI, WV, AK, CT, DC, ID, ME, MA, MS (second pass) — see `docs/state-rollout.md`'s table and each state's `docs/states/<code>.md`. DE/MT/NH/OR correctly excluded (no general sales tax). **Actually built and matching live data: only NC, GA, NJ.** Investigated-but-not-built is the normal end state for the rest today — nearly every one hit a real blocker (a placeholder tax body used by a real share of ship-tos, a confirmed live stale rate needing a human call, an undecoded jurisdiction-code type, a missing definition) documented in `docs/pending-business-decisions.md`. A small number (MD, IN, KY, MI, ME) came back genuinely clean — flat-rate states where A+ already matches the official source exactly with no blocker at all. |
| **3. Dashboard integration** | Feeding a state's comparison into the shared "Needs attention" / "Upcoming" tiles | NC, GA, and NJ feed it today. |
