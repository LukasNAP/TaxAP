...
| **2. A+ jurisdiction matching** | Pulling that state's real `XATXBD` tax-body data and determining (a) whether it needs address matching or maps 1:1 to jurisdictions, and (b) whether A+'s rate is actually comparable to the official one | **Milestone (2026-08-26, later session): investigated (data pulled, findings documented) for all 47 states/DC that need it** — NC, GA, SC, NJ, AL, AZ, MO, CO, LA, NY, HI, IA, KS, ND, SD, VT, NM, MN, NE, NV, OK, UT, WA (first pass) plus AR, WY, CA, TX, FL, PA, OH, TN, MD, IN, KY, MI, RI, IL, VA, WI, WV, AK, CT, DC, ID, ME, MA, MS (second pass) — see `docs/state-rollout.md`'s table and each state's `docs/states/<code>.md`. DE/MT/NH/OR correctly excluded (no general sales tax). **Actually built and matching live data (2026-08-26, later same session): NC, GA, NJ, MD, IN, KY, MI, ME, CT, MA, MS — 11 states.** The last 8 came back genuinely clean (a single flat statewide rate, no blocking finding) and were wired through one generic `server/flat-state-aplus.mjs` reconciler rather than eight near-duplicate files. Investigated-but-not-built is the normal end state for everything else today — nearly every remaining state hit a real blocker (a placeholder tax body used by a real share of ship-tos, a confirmed live stale rate needing a human call, an undecoded jurisdiction-code type, a missing definition) documented in `docs/pending-business-decisions.md`. |
| **3. Dashboard integration** | Feeding a state's comparison into the shared "Needs attention" / "Upcoming" tiles | NC, GA, NJ, MD, IN, KY, MI, ME, CT, MA, and MS feed it today (11 states), via a shared `FlatStateAplusPanel` for the 9 flat-rate states plus NC's own inbox shape and GA's boundary panel. |
