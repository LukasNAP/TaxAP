...
**Connected (9):** NC, GA, CA, TX, FL, PA, OH, TN, SC (SC's official source only — `server/sc-rates.mjs`, shipped 2026-08-26; A+ matching not built)
...
| NY | Publication 718 (tax.ny.gov) | PDF | Same PDF-table problem class as SC — would need a `pdftotext -table`-style approach (or confirm whether `server/sc-rates.mjs`'s approach reuses cleanly) rather than a new hand-rolled parser |
...
8. **Solve NY's PDF gap and hardcode MD.** NY is the same PDF-table problem class SC just solved; reusing `server/sc-rates.mjs`'s `pdftotext -table` approach may be a fast follow rather than new work. MD has no scrapable source at all (its FAQ page is a JS-rendered SPA) and no local tax to track — just hardcode the flat 6% rate, same treatment as CT/ME/MA/MS.
