...
Last updated: August 26, 2026 (later same-day session) — corrected a conflation that had built up in this doc between two different meanings of "investigated": **Layer 1 (does a validated official source exist)** has now been checked for all 51 jurisdictions, but **Layer 2 (has that state's real `XATXBD` A+ data actually been pulled and matched against the official source)** is still partial. Also fixed a stale claim below that Georgia wasn't wired into the dashboard — it is (`GeorgiaBoundaryPanel` in `app/page.tsx`), and New Jersey now is too (`server/nj-aplus.mjs`, `/api/official/states/NJ/aplus`). See `docs/state-rollout.md` for the full per-state Layer 2 findings and its status table.

## Where we actually stand

Getting all 51 jurisdictions into the "Needs attention" dashboard is not one task — it's three separable layers, and today's coverage is uneven across all three. **"Investigated" on its own is ambiguous — always say which layer:**

| Layer | What it means | Current coverage |
|---|---|---|
| **1. Official source** | A validated, machine-readable feed of that state's current rates | **All 51 jurisdictions have had their official source checked at least once.** 19 are `connected` in `official-source-registry.mjs` (NC, GA, CA, TX, FL, SC, PA, OH, TN, MD, NJ, AR, WY, IN, KY, MI, RI, NV, NE). 4 are `no-general-sales-tax` (DE, MT, NH, OR — excluded from comparison entirely, not unbuilt). 2 are `machine-readable-source` (IL, VA — real source confirmed, adapter not yet built). The remaining ~26 are `research-needed`, though most of those already have a real source *found* in the discovery passes below — "research-needed" here means no adapter has been written yet, not that nobody has looked. |
| **2. A+ jurisdiction matching** | Pulling that state's real `XATXBD` tax-body data and determining (a) whether it needs address matching or maps 1:1 to jurisdictions, and (b) whether A+'s rate is actually comparable to the official one | **Milestone (2026-08-26, later session): investigated (data pulled, findings documented) for all 47 states/DC that need it** — NC, GA, SC, NJ, AL, AZ, MO, CO, LA, NY, HI, IA, KS, ND, SD, VT, NM, MN, NE, NV, OK, UT, WA (first pass) plus AR, WY, CA, TX, FL, PA, OH, TN, MD, IN, KY, MI, RI, IL, VA, WI, WV, AK, CT, DC, ID, ME, MA, MS (second pass) — see `docs/state-rollout.md`'s table and each state's `docs/states/<code>.md`. DE/MT/NH/OR correctly excluded (no general sales tax). **Actually built and matching live data (2026-08-27): NC, GA, NJ, MD, IN, KY, MI, ME, CT, MA, MS, FL, PA, OH, VA, NY, AZ, AL — 18 states.** 11 came from the flat-state pass (2026-08-26); FL/PA/OH/VA/NY/AZ/AL (2026-08-27) came from a second generic reconciler, `server/direct-mapping-aplus.mjs`, for "many codes, each mapped directly to one real jurisdiction" states - 46 real confirmed findings now visible. MO and CO were investigated but not built (no official-source adapter existed for either before this session, and each revealed enough real complexity - MO's non-discoverable code numbering, CO's ambiguous multi-rate cities - to warrant its own dedicated pass rather than a rushed one). AR and TX were deliberately deferred (real city-to-county crosswalk and same-city-name ambiguity, respectively, that a first cut of this matcher wasn't built to handle safely). Investigated-but-not-built is the normal end state for everything else - nearly every remaining state hit a real blocker documented in `docs/pending-business-decisions.md`. |
| **3. Dashboard integration** | Feeding a state's comparison into the shared "Needs attention" / "Upcoming" tiles | **All 18 built states now feed it eagerly on dashboard load** (2026-08-27) via a new batch endpoint (`/api/official/findings`, `readAllWiredStateFindings()`) - previously only NC and GA fed the inbox without a drawer being opened first; the flat-rate and direct-mapping states' findings were invisible until a user manually opened that state. Uses a shared `FlatStateAplusPanel` for the 9 flat-rate states and a new `DirectMappingAplusPanel` for FL/PA/OH/VA/NY/AZ/AL, plus NC's own inbox shape and GA's boundary panel. |

Georgia's matching engine has run end to end against production data (2,293 matched + 92 unmatched + 19 ambiguous ship-tos, 30 of 130 tax bodies showing a real rate difference, cross-state assignments now separated out) and is visible on the dashboard via `GeorgiaBoundaryPanel`. New Jersey's is wired the same way via `/api/official/states/NJ/aplus`.

## Layer 1: official-source status, all 51 (per `official-source-registry.mjs`)

**Connected (18):** NC, GA, CA, TX, FL, PA, OH, TN, SC (official source only, `server/sc-rates.mjs`; A+ matching not built), AR, WY, NV (genuine drop-in SST states), IN, KY, MI, RI, NE (flat/no-local-tax-or-county SST states, special-cased), MD (hardcoded flat 6%, no local tax), NJ (cross-validated live against nj.gov, no local tax) — all shipped 2026-08-26. Note on NE: the initial "config-only drop-in" recommendation was wrong on two counts (real active county count, and a parser crash on alphanumeric special-district codes) — both only caught by actually running the adapter against live data before trusting the claim. See `docs/states/ne.md`.

**11 SST states investigated live 2026-08-26, 9 correctly held back (IA, KS, MN, ND, OK, SD, UT, VT, WA)** — each for a real, state-specific blocker documented in its own `docs/states/<code>.md`: several have a placeholder tax body covering a large share of ship-tos (`IA000` 55/262, `MN000` 113/387 with a likely systemic Twin-Cities-metro undercharge, `ND000` on literally 100% of ship-tos and worse than HI since ND's tax isn't optional, `SD000` 9/43, `VT000` 9/46, `UT000` 64/268), confirmed live stale rates (OK's Oklahoma City Canadian-County side, 8 years stale on ~29% of OK ship-tos; VT's Essex Junction and Waterbury; UT widespread across 60% of codes), or an undecoded/parser-breaking special-district code type (KS's alphanumeric type-79, 51.6% of the official file; WA's `L####` codes, now parseable but still structurally unresolved). None are ready to build without a human decision.
...
| NE | 31 | 93 | **Live-verified 2026-08-26 against production `XATXBD`, see `docs/states/ne.md`.** Local variation is real and city-driven as expected (only 1 of 93 counties — Dakota — has any county-level option tax at all), but no GA-style address matching is needed: A+'s `NE#####` codes are the real Census place FIPS code, directly joinable against Streamlined's city rows. 20/22 real codes match exactly; 1 confirmed live stale rate (Gering, 7.0% vs. official 7.5% since 2022); `NE000` DO-NOT-USE covers 18% of active ship-tos (needs a human decision, AL000/MN000-style); `NEDS000` ("Netherlands No Tax") is wrong-jurisdiction contamination, must be excluded explicitly. |
...
| HI | DOTAX county surcharge table (files.hawaii.gov) | PDF (small, 4 rows) | **RESOLVED 2026-08-27, per Lukas's explicit decision: Hawaii is excluded from the rate-comparison dashboard entirely**, the same as DE/MT/NH/OR. Hawaii's DOTAX states outright "Hawaii does not have a sales tax" — the General Excise Tax (GET) taxes business gross receipts, and the "rate" a customer sees is a voluntary capped pass-on amount, not a statutory transaction tax. No HI adapter should be built - see `server/official-source-registry.mjs` and `docs/pending-business-decisions.md`. |
...
5d. **After 5's zip support lands, most of the 11 unzipped states still need GA-style address/place matching, not a flat county total** — only NV looks close to a simple county-only case; IA/KS/ND/OK/SD/UT/VT/WA all have real city, special-district, or place-level variation confirmed in the 2026-08-26 pass. **NE re-verified live 2026-08-26 (see `docs/states/ne.md`) and is the exception: real city-level variation exists, but A+'s own `NE#####` codes are the real Census place FIPS code, directly joinable against Streamlined's city rows by code — no freeform address parsing needed, unlike GA.** One confirmed live stale-rate finding (Gering) and one DO-NOT-USE/contamination pair to handle first.
...
9. **Product decisions on the structurally-different-or-nonexistent states, all resolved 2026-08-27:** DE/MT/NH/OR (confirmed no general sales tax, excluded entirely), **HI (excluded entirely - GET is not a buyer-facing sales tax)**, and **AK (excluded entirely - real local-only tax exists, but no state tax and no source ever covers every jurisdiction, so a comparison could never be complete)**. NM (GRT, seller-side legal incidence) remains treated as functionally equivalent to a normal sales tax per Lukas's earlier decision - unaffected by the HI/AK exclusions, still open on its own live-data findings (see `docs/pending-business-decisions.md`).
