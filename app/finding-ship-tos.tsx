...
type Result = { total: number; page: number; pageSize: number; retrievedAt: string; rows: { companyNumber: string; customerNumber: string; shipToNumber: string }[] };
...
function ShipToList({ apiBase, taxBody, state, scope, expectedCount }: Selection) {
  ...
  return <div>
    <p>{scope === "rate-risk" ? "Active ship-tos assigned to this tax body with tax treatment 0, across all states, matching the finding's rate-risk count." : `Active ship-tos assigned to this tax body in ${state}.`} Company, customer and ship-to numbers only. Use these numbers to locate records in A+.</p>
    {error ? <div role="alert"><p>{error}</p><button className="secondary-button" type="button" onClick={() => { setError(""); setResult(null); setAttempt(attempt + 1); }}>Retry</button></div> : !result ? <p role="status">Loading ship-tos…</p> : <>
      ...
      {result.rows.length === 0 ? <p>No assignments on this page. {page > 0 && "Return to the first page to refresh the list."}</p> : <div className="table-scroll"><table className="coverage-table"><caption>Ship-tos assigned to {taxBody}</caption><thead><tr><th scope="col">Company number</th><th scope="col">Customer number</th><th scope="col">Ship-to number</th></tr></thead><tbody>{result.rows.map((row, index) => <tr key={`${row.companyNumber}:${row.customerNumber}:${row.shipToNumber}:${index}`}><td>{row.companyNumber}</td><td>{row.customerNumber}</td><td>{row.shipToNumber || "(blank)"}</td></tr>)}</tbody></table></div>}
