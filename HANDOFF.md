...
Last updated: August 31, 2026 (apdock01 deployment preparation is built and verified, but not started. The private GitHub repository was cloned to `/var/atlanticapps/taxap` using a dedicated read-only deploy key; `docker compose build` completed successfully on apdock01, including npm dependency installation and the production application build. The image runs the browser-facing web app and the read-only A+ connector in one private application container; only the Nginx HTTPS proxy publishes a configurable host port, and it forwards `/api/*` privately to the connector so port 3001 and database connectivity are never browser-accessible. Production browser requests use the same-origin proxy rather than a public connector URL. `deployment/apdock01/` includes a Compose file, Nginx config, secret-free environment template, and runbook; no `.env`, certificate, token, or A+ credential is in Git. Compose now reserves a read-only mount for the required Entra service-principal PEM at `/run/secrets/azure-sql-client-cert.pem`; the source file must exist only on apdock01 under `secrets/`. Container runtime includes `poppler-utils` for South Carolina's official PDF adapter. `npm test` passes 141 tests; `npm run lint` and `git diff --check` pass. Before starting the stack, create a noninteractive least-privilege Entra service principal, create its contained read-only Azure SQL user, and place its combined PEM on apdock01. No A+ reads or writes, container start, or production deployment occurred.)

Last updated: August 28, 2026, later session (Missouri was re-investigated live, read-only. The current July–September 2026 Missouri DOR XLSX was fetched and parsed (2,536 jurisdiction rows), correcting the earlier overstatement that all MO codes were arbitrary: 42 named active A+ bodies use a three-digit DOR county/equivalent segment (366 ship-tos), 15 use a city segment, 5 concatenate city+county, one strips leading zeroes, and 24 legacy-looking codes (82 ship-tos) have no safe direct code relationship. The state snapshot has 515 active Missouri-address ship-tos: 477 on 87 named MO bodies, 34 missing `MO000` assignments, and four cross-state bodies. Several A+ totals exactly match the DOR sales rate, but `MO0029` St. Louis County (16 ship-tos) and `MO51572145` Neosho match the DOR use rate instead; `MO0229` Wright County matches neither. Aggregate invoice history then confirmed all three have actually been charged at 4.225% in 2026 (34, 1, and 2 invoices respectively), so this is live behavior rather than dormant configuration; the available HSHED invoice-header tax-type fields are blank for all 37 records, so the database has no useful header-level sales/use indicator. Because Missouri DOR's final CID/TDD special-district segment is absent from A+'s code, TaxAP cannot choose a single official sales/use/special rate safely. No Missouri matcher was built; `docs/states/mo.md`, `docs/state-rollout.md`, and `docs/pending-business-decisions.md` now document the verified pattern and the business decision needed. No A+ writes, commit, push, or deployment occurred.)

Last updated: August 28, 2026, later session (Colorado is now built and wired through `server/co-rates.mjs` and `server/co-aplus.mjs`. Colorado DOR's current July–December 2026 workbook is discovered from its landing page—not a hardcoded filename—and parsed into 447 official location totals. The matcher honors Lukas's decision to monitor the **assigned code only**: six-digit state jurisdiction-code tails match directly; short A+ city descriptions match only a unique official city. Multi-rate city/district cases (Aurora, Colorado Springs, Lone Tree, Littleton, Broomfield, and others) are reported unmatched rather than arbitrarily assigned to an official variant. Live aggregate-only validation found 9 confirmed rate differences affecting 114 ship-tos: Denver (62; 8.81% vs 9.15%), Boulder (10; 8.845% vs 9.195%), Fort Collins (9; 7.55% vs 8.3%), Commerce City (8), Grand Junction (8), Loveland (8), Englewood (6), Castle Rock (2), and Aspen (1). 116 ship-tos remain intentionally unmatched; 8 missing/special-category assignments are visible exclusions. `npm test` passes 140 tests and `npm run lint` passes. No A+ writes, commit, push, or deployment occurred.)

Last updated: August 28, 2026, later session (California A+ tax-body comparison is now built and wired through `server/ca-aplus.mjs`. This is a constrained **tax-body configuration** check, not address assignment validation: A+'s own description must explicitly identify one CDTFA county or one unique CDTFA city; no customer address, ZIP, or county is inferred. A live aggregate-only run compared 1,327 CA ship-tos, found 46 confirmed rate differences affecting 446 ship-tos, and left zero comparable rows unmatched. `CA000`/`CA001`/`CA003` missing definitions (236 ship-tos) and `CA1034E` equipment category (1) are visible exclusions; 11 cross-state assignments are separately counted. Largest current result: Los Angeles County (`CA1070`), A+ 10.25% vs CDTFA 9.75% for 202 active ship-tos. The existing address-boundary follow-on remains necessary to validate whether individual ship-tos are assigned to the correct tax body. `npm test` passes 137 tests and `npm run lint` passes. No A+ writes, commit, push, or deployment occurred.)

Last updated: August 28, 2026, later session (Texas A+ matching is now built and wired through `server/tx-aplus.mjs`. It uses the Texas Comptroller's existing official city/local totals and performs a deliberately constrained code-level comparison: explicit county hints match that county, bare-city codes only compare when every official county variant currently has the same rate, and divergent candidates remain unmatched unless A+'s rate matches none of them. This surfaces confirmed differences without assigning an address to an arbitrary county. Live aggregate-only validation found four confirmed TX differences: Midland (`TX1421`, 9 ship-tos, 8.0% vs 8.25%), Hockley (`TX1005`, 6, 8.25% vs 7.25%), Cleburne (`TX351`, 5, 7.75% vs 8.25%), and The Colony (`TX2220`, 4, 7.75% vs 8.25%). Twenty cross-state and two missing-definition/misinput ship-tos stay visibly excluded; 98 ship-tos remain unmatched rather than guessed. Arkansas was re-evaluated and remains honestly blocked from a full city-level matcher: its official SST data separates county/city components, while TaxAP has no verified city-to-county crosswalk and must not invent one. `npm test` passes 134 tests and `npm run lint` passes. No A+ writes, commit, push, or deployment occurred.)

Last updated: August 28, 2026 (Tax-treatment impact is now applied **per tax body**, not merely shown as a global dashboard context panel. The read-only `/api/aplus/tax-treatment` aggregate query returns each assigned tax body's counts by treatment code without customer/address/order/invoice identity. Dashboard rate-risk impact now includes only `0` (always taxable) ship-tos; `3` (never taxed) stays visible but is excluded from rate-risk impact; and `J` is visibly identified as requiring line-level review rather than being given a header-level conclusion. New York's official-source refresh failure was root-caused as a missing host-installed `pdftotext` executable, not an A+ connection issue. `server/pdf-utils.mjs` now uses the pinned in-process `pdfjs-dist` package, preserving physical PDF table lines. Live public-source validation against NY Publication 718 succeeded: 76 rate rows, effective March 1, 2025. This does not query or modify A+; restart the local connector before expecting its already-running process to load this change. `npm test` passes 130 tests and `npm run lint` passes.)
...
- Test suite: 140 passing tests as of 2026-08-28 (grows most sessions — check `npm test` output rather than trusting this number for long).
...
| `server/ca-aplus.mjs` | Constrained aggregate California tax-body configuration reconciliation; preserves visible missing-definition, equipment-category, and cross-state exclusions |
| `server/co-rates.mjs` | Colorado DOR current-period workbook discovery and hardened official location-total parser |
| `server/co-aplus.mjs` | Constrained Colorado assigned-tax-body reconciliation; exact state codes and unique official city names only |
