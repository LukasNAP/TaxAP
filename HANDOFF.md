...
Last updated: August 26, 2026, later session (Layer 2 A+ matching investigated for the remaining 24 states - AR/WY/CA/TX/FL/PA/OH/TN/MD/IN/KY/MI/RI/IL/VA/WI/WV/AK/CT/DC/ID/ME/MA/MS - completing Layer 2 for all 51 jurisdictions; a generic `server/flat-state-aplus.mjs` reconciler plus new `server/me-rates.mjs`/`ct-rates.mjs`/`ma-rates.mjs`/`ms-rates.mjs` adapters were then built and wired into the dashboard for the 8 states confirmed clean with zero blocking finding - MD, IN, KY, MI, ME, CT, MA, MS - generalizing `app/page.tsx`'s NJ-only flat-state panel to cover all 9 flat-rate states via `FLAT_STATE_APLUS_STATES`; found and fixed a Node TLS trust-store gap for `dor.ms.gov`'s legitimate cert (`--use-system-ca` added to `npm run dev:connector`); 21 new pending business decisions documented for the rest of the 24-state batch - see `docs/pending-business-decisions.md`)
...
- Test suite: 102 passing tests as of 2026-08-26, later session (grows most sessions — check `npm test` output rather than trusting this number for long).
...
- Two small, reusable read-only scripts added for future Layer 2 investigations instead of hand-rolled OPENQUERY SQL: `scripts/investigate-state.mjs <CODE>` (wraps `readStateDetail`) and `scripts/fetch-official-rates.mjs <CODE>` (wraps whichever official adapter is wired for that state).
