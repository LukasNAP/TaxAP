...
Last updated: August 31, 2026 (fixed two dashboard reliability issues without contacting A+. The dedicated **Needs attention** view and its navigation badge now use the same completed all-state, tax-treatment-aware rate-risk inbox as the home dashboard; while Georgia and the other state comparisons are loading, the page shows an explicit loading state instead of an NC-only partial count. Georgia boundary reconciliation no longer chooses a jurisdiction from input order when one tax body is assigned to multiple Georgia jurisdictions. This was the cause of `NC060` alternating between FIPS 067 and 135 across otherwise identical calls: the aggregate's tied “majority” sort had no deterministic result. Mixed assignments now retain their aggregate count but return no jurisdiction or official rate, visibly marked as needing configuration review rather than guessed. A regression test reverses the input order and requires the same result. `npm test` passes 141 tests; `npm run lint` and `git diff --check` pass. No A+ reads or writes, commit, push, or deployment occurred.)
