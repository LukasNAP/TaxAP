...
Last updated: August 31, 2026 (prepared, but did not deploy, an apdock01 Docker/Nginx hosting layout. The image runs the browser-facing web app and the read-only A+ connector in one private application container; only the Nginx HTTPS proxy publishes a configurable host port, and it forwards `/api/*` privately to the connector so port 3001 and database connectivity are never browser-accessible. Production browser requests use the same-origin proxy rather than a public connector URL. Added a deployment-specific Compose file, Nginx config, secret-free environment template, and runbook under `deployment/apdock01/`; no `.env`, certificate, token, or A+ credential is in the repository. Container runtime now includes `poppler-utils` for South Carolina's official PDF adapter. `npm test` passes 141 tests; `npm run lint` and `git diff --check` pass. A local Docker build reached dependency installation but was stopped by the local Docker environment's untrusted corporate TLS certificate for npm; certificate validation was deliberately not disabled. Before a build on apdock01, IT must make the corporate root CA available to Docker and provision a noninteractive, least-privilege Microsoft Entra identity with read-only Azure SQL/DWStage access. No A+ reads or writes, commit, push, or deployment occurred.)
