...
## Ship-to address visibility fix (deployed September 23)
...
Production build, lint, TypeScript and302 existing tests passed. Browser-checked the real CSS with a synthetic five-field ship-to table at1040px and420px panel widths: full multiline address and ship-to number visible, with table semantics retained in the accessibility tree. This was a synthetic layout check, not a hosted/customer-data acceptance test. User authorized delivery: implementation `eaf8cc1` committed/pushed/deployed. Rebuilt and replaced app only using both Compose files. Verified host baseline (page matched5218b8b;41e6eeb banner cleanup also included), uploaded hashes, protected backup `/var/atlanticapps/taxap-backups/shipto-layout-20260923-154544` and rollback image `taxap-rollback:before-shipto-layout`. Hosted page200 and its served CSS confirm1040px panel and container-query rules; field labels present in bundle. Review integrity passed, public root302/API401, four containers running with zero restarts. Fresh production build and302 tests passed. Signed-in user visual acceptance remains pending; no customer details queried or printed.
...
Operational switches (restart required): `TAXAP_SQL_SHARED_POOL=false` restores per-request pools; `TAXAP_SQL_POOL_MAX` defaults5; `TAXAP_SQL_TIMING_LOG=true` enables aggregate per-query measurements. Existing settings and SQL03 trust exception unchanged. Tests cover concurrent first-use, idempotent release, expiry drain, connection error replacement, connection failure retry and timing-log privacy. Build,302 tests,lint,TypeScript passed. User authorized commit,push,deploy. Implementation `50e3eab` pushed and deployed. Host baseline and uploaded hashes verified; protected backup `/var/atlanticapps/taxap-backups/shared-sql-pool-20260923-093954`, rollback image `taxap-rollback:before-shared-sql-pool`. Built with both Compose files and replaced app only. Three concurrent hosted coverage reads returned200 with51 states and reconciled2,813 exclusions. Hosted all-state batch completed200 in33.4 seconds with47 checks and no failed states (not a before/after performance benchmark). Separate in-container SQL-login check ran4 SELECT1 queries across multiple/released leases with1 pool creation. Review integrity passed; root302/API401; four containers running with zero restarts. Hosted Entra/Windows paths and signed-in browser behavior were not exercised. No SQL queries or A+ data changed.
...
Review corrections: unknown codes are not labeled foreign; prefixes never prove actual tax treatment or destination. Separate-read consistency checks cover each category as well as totals. Optional query failure retains main coverage and reports unavailable; new error log is generic. Added injected connection seam and tests for failure isolation/cleanup, matched and mismatched categories, aggregation and privacy. Production build, all 295 tests, lint, TypeScript and whitespace checks passed. User authorized commit, push and deploy; host target files match the committed baseline. Implementation `94aef16` committed/pushed and deployed with user authorization. Protected source/configuration and consistent SQLite backups: `/var/atlanticapps/taxap-backups/excluded-tax-bodies-20260923-092304`; rollback image `taxap-rollback:before-excluded-tax-bodies`. Uploaded hashes verified; build used both Compose files and only app was replaced. Hosted coverage response200: 2,813 exclusions, tax-body patterns total107 U.S. prefixes, 2,705 other codes, 1 ZTEMP, 0 blank. Grand total and all three state-value category totals reconcile; response field/prefix allowlist checks passed. New UI bundle verified. Review database integrity passed; public root302, anonymous API401, four containers running with zero restarts. Signed-in browser visual acceptance remains untested. No ERP writes or reassignment.
...
September 23 deployment verified: implementation committed/pushed as `71285b3`. Fresh production build, 282 tests, lint and TypeScript passed. Host page/styles matched the prior baseline; four application files transferred with SHA-256 verification. Protected configuration/source and consistent SQLite backups saved under `/var/atlanticapps/taxap-backups/jurisdictions-20260923-081211`; rollback image `taxap-rollback:before-jurisdictions`. Built with both Compose files and replaced only taxap-app. Deployed bundle contains the nationwide inventory view. Private router returned 200 for the page, CA official inventory (541 rates), CA comparison (150 assignment groups/143 comparison rows), FL comparison (68/59), and GA boundary (126 comparison rows). Review integrity passed. Public HTTPS root redirects (302), anonymous inventory API is denied (401), and all four containers run with zero restarts. These checks verify representative endpoint data, not every state or signed-in browser acceptance. Existing host modifications and credentials were preserved; no A+ writes.
...
Validation: production build, 282 tests, lint and TypeScript passed; local offline browser verified all state options, California filtering/unavailable notice, and Delaware classification. Full nationwide loading and signed-in hosted visual acceptance were not exercised; representative hosted endpoint checks are recorded above. Opening the view performs additional read-only source/comparison requests, bounded to three concurrent requests; subsequent visits reload, and counts explicitly describe loaded rows rather than total national coverage. Implementation is now committed, pushed and deployed as recorded above.

## Customer names and ship-to addresses added locally (September 22)

Deployment verified: implementation committed/pushed as 86335df, then deployed with explicit user authorization. Three host files matched the parent baseline; protected backups saved under `/var/atlanticapps/taxap-backups/shipto-details-*` and image tagged `taxap-rollback:before-shipto-details`. Compose validation/build succeeded, only app replaced. Deployed router endpoint returned 200 with 50 rows, expected field allowlist/address shape, populated names/addresses (presence booleans only in output), and no-store. Review integrity passed. Public root 302 and anonymous endpoint 401; four containers running without restarts. No customer details printed, no A+ writes. Browser visual acceptance remains for the signed-in user. This supersedes pending-deployment statements below.
